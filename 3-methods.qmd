...
The scoping review followed the Manual for Evidence Synthesis (Chapter 10 - Scoping reviews) from the Joanna Briggs Institute (JBI) and reported according to the Preferred Reporting Items for Systematic Reviews and Meta-analyzes extension for scoping review (PRISMA-ScR) guidelines.[@Tricco2018; @Peters2024]

### Concepts and Definitions {#sec-concepts}
The review focused exclusively on studies using CGM data as input for analyzes of associations with, or prediction of, \acr{CVD}.
...
| &emsp;&emsp;a) The association between CGM-derived metrics of glycemic control and cardiovascular risk markers or cardiovascular diseases (prevalent or incident). | Studies including CGM-derived metrics as outcomes. |
| &emsp;&emsp;b) CGM-derived metrics of glycemic control as predictors of cardiovascular risk markers or cardiovascular diseases (prevalent or incident). | Studies not focusing on CVD outcomes according to our definition, as outlined in @sec-concepts. |
|  | Studies focusing on pregnant women with any form of diabetes including gestational diabetes. |
|  | Studies where participants were monitored post-surgery or during hospitalization (e.g., Intensive care unit). |
...
Days with less than 70% of CGM data were removed in addition to participants with less than one day of data from both datasets to ensure sufficient data for the analyzes.
...
Illustration of how CGMformer is used in this analyzis.
:::

The CGMformer gives a 128-dimensional embedding vector for each 24-hour CGM data input.
Embeddings are numerical vectors that summarize the characteristics of the input glucose profile.
A simple vector can be drawn as an arrow in a 2-dimensional and a 3-dimensional space as shown in @fig-cancan-embedding.

::: {#fig-cancan-embedding fig-scap='Embeddings shown as 2-dimensional and 3-dimensional vectors.' layout-ncol=2 layout-valign="center"}
...
The cosine similarity is calculated by measuring the cosine angle between two vectors as shown by $\theta$ in @fig-cancan-embedding. The cosine similarity is a number between -1 and 1, where 1 means the two vectors are identical, 0 means they are unrelated, and -1 means that they are pointing in opposite directions (@fig-cancan-co_sim_emb). For the analyzes in this study, cosine distance, calculated as 1 − cosine similarity, was used because it expresses similarity as a distance measure, allowing embeddings with similar orientations to have small distances and facilitating downstream analyzes and visualizations. The cosine distances range from 0 to 2 where 0 means the two vectors are pointing in the same direction, 1 means they are unrelated, and 2 means that they are pointing in opposite directions.

::: {#fig-cancan-co_sim_emb fig-scap='Different cosine similarities.'}
...
Glucose variability strata is used as an example to illustrate the cosine distance analyzis of the within and between cosine distance pairs.
:::
...
The threshold-based categorizations for TIR, MAGE, and HbA1c were used solely for the cosine distance analyzes and to facilitate interpretation of the \acr{UMAP} visualizations and were not incorporated into model development or any statistical analyzes.
...
The following section describes the cardiovascular risk assessment analyzes, in which \acr{NT-proBNP} was predicted as a binary outcome using a >125 pg/mL threshold.[@mcdonagh2021; @Heidenreich2022; @Welsh2022]
...
![](figures/3_methods_fig/study_2_bootstrap.png){width=70%}
...
::: {.content-visible when-format="pdf"}
```{=latex}
\tablenote[0.75\linewidth]{
    \textsuperscript{*}Clinical variables are the same as those included in the SCORE2-Diabetes risk model: Age, sex, smoking status, systolic blood pressure, HDL cholesterol, total cholesterol, diabetes duration, HbA1c, and eGFR.\newline
    \textit{Abbreviations:} TIR = time in range; MAGE = mean amplitude of glycemic excursions; HbA1c = hemoglobin A1c; eGFR = estimated glomerular filtration rate
}
```
:::
...
<p style="font-size:75%">
    \* Clinical variables are the same as those included in the SCORE2-Diabetes risk model: Age, sex, smoking status, systolic blood pressure, HDL cholesterol, total cholesterol, diabetes duration, HbA1c, and eGFR.\
    *Abbreviations:* TIR = time in range; MAGE = mean amplitude of glycemic excursions; HbA1c = hemoglobin A1c; eGFR = estimated glomerular filtration rate
</p>
...
![](figures/3_methods_fig/study3_pred_horizon.png){width=75%}

The prediction horizon and overview of the creation of samples
...
3.	**Base-Generalized:** Here, for each participant, the 11 corresponding training datasets described above were used to generate 11 separate models, which were all subsequently tested on the given participant’s last week of CGM data.
...
::: {#fig-study3_SEG fig-scap='Surveillance error grid' width=65%}
![](figures/3_methods_fig/study3_SEG_empty.png){width=65%}

Surveillance error grid showing the clinical risk associated with prediction errors. The green region indicates no clinical risk, whereas yellow, orange, and red regions represent increasing levels of clinical risk. Generated from the methcomp package.[@doorn_wptmdoornmethcomp_2026]
