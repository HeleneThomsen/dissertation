...
::: {#fig-cancan-embedding layout-ncol=2 layout-valign="center"}
...
\acr{UMAP} algorithm was used to reduce the dimensionality of the embeddings to two dimensions since a 128-dimensional vector is not easily visualized. The concept of dimensionality reduction is illustrated in @fig-dim_reduce.
...
The third study investigated racial bias in a data driven machine learning model only based on blood glucose measurements from a CGM device.
...
Three \acr{LSTM} models and the \acr{LOCF} as a naive baseline.
